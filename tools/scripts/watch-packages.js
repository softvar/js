/**
 * Source - https://github.com/Dombo/node-lerna-monorepo
 * URL - https://github.com/Dombo/node-lerna-monorepo
...
const bootstrap = () => {
  try {
    const lernaArgs = `bootstrap --scope ${
      manifest().name
    } --include-filtered-dependencies`.split(" ");
    execa("lerna", lernaArgs, { stdio: "inherit" });
  } catch (error) {
    throw new Error(
      `There was a problem trying to bootstrap the tree ${error}`
    );
  }
};
...
const buildDependencyTreeMeta = async () => {
  try {
    ...
    throw new Error(
      `There was an error building the dependency tree: ${error}`
    );
  }
};
...
const buildDependencyChain = async path => {
  ...
  const buildOperations = buildWorthyParents.map(dependency =>
