 ...
 */
export function getQueryParamValue(url: string, queryParam: string) {
  if (!isString(url) || !isString(queryParam)) {
    return '';
  }
 ...
 */
export function removeURLParameter(url: string, parameter: string | number) {
  if (!isString(url) || !isString(parameter)) {
    return url;
  }
